...
# ------------------------------------------------------------
# Rotating MNIST Dataset Loader
# ------------------------------------------------------------
def get_rotating_mnist(batch=128):
    # Randomly rotate each image in [0, 360) degrees
...
# ------------------------------------------------------------
# ConvNet for 28×28 Rotating MNIST
# ------------------------------------------------------------
...
def plot_all_3d(opt):
    ...
    scat = ax.scatter(step, diff, mom,
                      c=step_raw,
                      s=70,
                      alpha=0.9)

    fig.colorbar(scat, shrink=0.55)
    #ax.set_title("Scatter: Step Size vs Gradient Diff. vs Momentum")
    ax.set_xlabel("ηₜ (Step Size)")
    ax.set_ylabel("‖Δgₜ‖")
    ax.set_zlabel("‖mₜ‖")
...
# TRAINING LOOP
# ------------------------------------------------------------
def train_model(name, opt_fn, epochs=70):

    device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
    train_loader, test_loader = get_rotating_mnist()
...
# ------------------------------------------------------------
# MAIN Comparison Pipeline for Rotating MNIST
# ------------------------------------------------------------
def compare_all_rotating_mnist(epochs=70):
    ...
    print("\n=== Running Adam ===")
    results["Adam"] = train_model("Adam", lambda p: torch.optim.Adam(p, lr=0.001), epochs)
    ...
    plot_metric(0, "Rotating MNIST: Train Accuracy", "Accuracy (%)")
    plot_metric(1, "Rotating MNIST: Test Accuracy",  "Accuracy (%)")
    plot_metric(2, "Rotating MNIST: Train Loss",     "Loss")
    plot_metric(3, "Rotating MNIST: Test Loss",      "Loss")
    ...
    plt.xticks(x, names)
    #plt.title("Rotating MNIST: Precision / Recall / F1")
    plt.legend()
    plt.show()
...
if __name__ == "__main__":
    compare_all_rotating_mnist(epochs=70)
