...
# TRAINING LOOP
# ------------------------------------------------------------
def train_model(name, opt_fn, epochs=100):
...
# ------------------------------------------------------------
def compare_all_usps(epochs=100):
    ...
    print("\n=== Running SGD ===")
    results["SGDM"] = train_model("SGDM", lambda p: torch.optim.SGD(p, lr=0.01, momentum=0.9), epochs)
    ...
    plot_metric(0, "USPS: Train Accuracy", "Accuracy (%)")
    plot_metric(1, "USPS: Test Accuracy",  "Accuracy (%)")
    plot_metric(2, "USPS: Train Loss",     "Loss")
    plot_metric(3, "USPS: Test Loss",      "Loss")
    ...
    plt.xticks(x, names)
    #plt.title("USPS: Precision / Recall / F1")
    plt.legend()
    plt.show()
